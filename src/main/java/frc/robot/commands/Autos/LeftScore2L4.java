...
public class LeftScore2L4 
{
    public static Command create(boolean isRed, Drivetrain drivetrain, FieldMap map, Localizer localizer, CoralEndeffector endEffector, CoralElevator elevator, Lidar lidar)  
    {
        ...
        Pose2d tag6Pose = map.getTagRelativePose(6, -1, new Transform2d(0.375, 0, new Rotation2d(Math.PI)));
        ...
        Pose2d tag19Pose = map.getTagRelativePose(19, -1, new Transform2d(0.375, 0, new Rotation2d(Math.PI)));
        ...
        if (isRed)
        {
            ...
            segments3.add(new Segment(redI1, tag6, tag6Pose.getRotation().getRadians(), 1.5));

            path1 = new Path(segments1, tag11Pose.getRotation().getRadians());
            path2 = new Path(segments2, tag1Pose.getRotation().getRadians());
            path3 = new Path(segments3, tag6Pose.getRotation().getRadians());
        }
        else
        {
            ...
            segments3.add(new Segment(blueI1, tag19, tag19Pose.getRotation().getRadians(), 1.5));

            path1 = new Path(segments1, tag20Pose.getRotation().getRadians());
            path2 = new Path(segments2, tag13Pose.getRotation().getRadians());
            path3 = new Path(segments3, tag19Pose.getRotation().getRadians());
        }
