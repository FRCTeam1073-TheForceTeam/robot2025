...
public class CANdleObserver extends Command {
  ...
  public void execute() {
    ...
    if (endeffector.getHasCoral()){
      candleControl.setRGB(255, 255, 255, candleNum + 2 * (numPerStrip + 3), numPerStrip * 2);//sides of funnel - light on
    }
    else{
      candleControl.setRGB(0, 0, 0, candleNum + 2 * (numPerStrip + 3), numPerStrip * 2);//sides of funnel - light off
    }
    ...
    if (climber.getIsDisengaged()){
      ...
      candleControl.setRGB(255, 0, 255, candleNum + numPerStrip + 3, numPerStrip + 3);//elevator side - purple
    }
    else{
